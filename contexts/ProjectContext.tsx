...
function projectReducer(state: ProjectState, action: ProjectAction): ProjectState {
  switch (action.type) {
    ...
    case 'UPDATE_CHAPTER':
      return {
        ...state,
        chapters: state.chapters.map(c => c.id === action.payload.id ? action.payload : c)
      }
    case 'DELETE_CHAPTER':
      return {
        ...state,
    ...
    case 'UPDATE_SYNOPSIS':
      return {
        ...state,
        synopses: state.synopses.map(s => s.id === action.payload.id ? action.payload : s)
      }
...
interface ProjectContextType {
  ...
  updateProject: (id: string, data: Partial<Omit<Project, 'id' | 'created_at' | 'updated_at'>>) => Promise<void>
  deleteProject: (id: string) => Promise<void>
  setCurrentProject: (project: Project | null) => void
  
  // Chapter actions
  createChapter: (data: Omit<Chapter, 'id' | 'created_at' | 'updated_at'>) => Promise<void>
  updateChapter: (id: string, data: Partial<Omit<Chapter, 'id' | 'created_at' | 'updated_at'>>) => Promise<void>
  deleteChapter: (id: string) => Promise<void>
  
  // Scene actions
  createScene: (data: Omit<Scene, 'id' | 'created_at' | 'updated_at'>) => Promise<void>
  updateScene: (id: string, data: Partial<Omit<Scene, 'id' | 'created_at' | 'updated_at'>>) => Promise<void>
  deleteScene: (id: string) => Promise<void>
  
  // Character actions
  createCharacter: (data: Omit<Character, 'id' | 'created_at' | 'updated_at'>) => Promise<void>
  updateCharacter: (id: string, data: Partial<Omit<Character, 'id' | 'created_at' | 'updated_at'>>) => Promise<void>
  deleteCharacter: (id: string) => Promise<void>
  
  // Synopsis actions
  createSynopsis: (data: Omit<Synopsis, 'id' | 'created_at' | 'updated_at'>) => Promise<void>
  updateSynopsis: (id: string, data: Partial<Omit<Synopsis, 'id' | 'created_at' | 'updated_at'>>) => Promise<void>
...
export function ProjectProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateProject = async (id: string, data: Partial<Omit<Project, 'id' | 'created_at' | 'updated_at'>>) => {
    dispatch({ type: 'SET_LOADING', payload: true })
    try {
      const { data: project, error } = await supabase
        .from('projects')
  ...
  const createChapter = async (data: Omit<Chapter, 'id' | 'created_at' | 'updated_at'>) => {
    dispatch({ type: 'SET_LOADING', payload: true })
    try {
  ...
  const updateChapter = async (id: string, data: Partial<Omit<Chapter, 'id' | 'created_at' | 'updated_at'>>) => {
    dispatch({ type: 'SET_LOADING', payload: true })
    try {
      const { data: chapter, error } = await supabase
        .from('chapters')
  ...
  const createScene = async (data: Omit<Scene, 'id' | 'created_at' | 'updated_at'>) => {
    dispatch({ type: 'SET_LOADING', payload: true })
    try {
  ...
  const updateScene = async (id: string, data: Partial<Omit<Scene, 'id' | 'created_at' | 'updated_at'>>) => {
    dispatch({ type: 'SET_LOADING', payload: true })
    try {
      const { data: scene, error } = await supabase
        .from('scenes')
  ...
  const createCharacter = async (data: Omit<Character, 'id' | 'created_at' | 'updated_at'>) => {
    dispatch({ type: 'SET_LOADING', payload: true })
    try {
  ...
  const updateCharacter = async (id: string, data: Partial<Omit<Character, 'id' | 'created_at' | 'updated_at'>>) => {
    dispatch({ type: 'SET_LOADING', payload: true })
    try {
      const { data: character, error } = await supabase
        .from('characters')
  ...
  const createSynopsis = async (data: Omit<Synopsis, 'id' | 'created_at' | 'updated_at'>) => {
    dispatch({ type: 'SET_LOADING', payload: true })
    try {
      const { data: synopsis, error } = await supabase
        .from('synopses')
        .insert([data])
        .select()
        .single()
      ...
      dispatch({ type: 'SET_ERROR', payload: error instanceof Error ? error.message : 'Failed to create synopsis' })
    } finally {
      dispatch({ type: 'SET_LOADING', payload: false })
    }
  }

  const updateSynopsis = async (id: string, data: Partial<Omit<Synopsis, 'id' | 'created_at' | 'updated_at'>>) => {
    dispatch({ type: 'SET_LOADING', payload: true })
    try {
      const { data: synopsis, error } = await supabase
        .from('synopses')
      ...
      dispatch({ type: 'SET_ERROR', payload: error instanceof Error ? error.message : 'Failed to update synopsis' })
    } finally {
      dispatch({ type: 'SET_LOADING', payload: false })
    }
  }
  ...
  const deleteSynopsis = async (id: string) => {
    ...
    try {
      ...
      dispatch({ type: 'SET_ERROR', payload: error instanceof Error ? error.message : 'Failed to delete synopsis' })
    } finally {
      dispatch({ type: 'SET_LOADING', payload: false })
    }
  }
  ...
  const loadProjectData = async (projectId: string) => {
    ...
    try {
      ...
      dispatch({ type: 'SET_ERROR', payload: error instanceof Error ? error.message : 'Failed to load project data' })
    } finally {
      dispatch({ type: 'SET_LOADING', payload: false })
    }
  }
