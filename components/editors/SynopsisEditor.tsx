...
export function SynopsisEditor({ synopsisId, onBack }: SynopsisEditorProps) {
  ...
  const project = synopsis
    ? state.projects.find((p) => p.id === synopsis.project_id)
    : null;

  // Estatísticas do projeto relacionado
  const projectStats = project
    ? {
        chapters: state.chapters.filter((c) => c.project_id === project.id)
          .length,
        scenes: state.scenes.filter((s) => {
          const chapter = state.chapters.find((c) => c.id === s.chapter_id);
          return chapter?.project_id === project.id;
        }).length,
        characters: state.characters.filter((c) => c.project_id === project.id)
          .length,
      }
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {isEditing
              ? "Edite o título e conteúdo da sinopse"
              : "Visualize e gerencie o conteúdo da sinopse"}
          </CardDescription>
        </CardHeader>
            ...
            <>
              ...
              <div>
                ...
                <div className="flex justify-between text-xs text-muted-foreground mt-2">
                  <span>
                    {getWordCount(editedSynopsis.content || "")} palavras
                  </span>
                  <span>
                    {getCharacterCount(editedSynopsis.content || "")} caracteres
                  </span>
                </div>
              </div>
            </>
            ...
            <div className="space-y-4">
              {synopsis.content ? (
                ...
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p>
                    Nenhuma sinopse ainda. Clique em "Editar" para adicionar o
                    conteúdo da sinopse.
                  </p>
                </div>
              )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              ~{getReadingTime(synopsis.content || "")}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              última atualização:{" "}
              {new Date(synopsis.updated_at).toLocaleDateString("pt-BR")}
            </p>
          </CardContent>
        </Card>
        ...
        <CardContent className="text-blue-800">
          <ul className="space-y-2 text-sm">
            <li className="flex items-start">
              ...
              <span>
                Mantenha entre 250-500 palavras para uma sinopse concisa
              </span>
            </li>
            <li className="flex items-start">
              <span className="w-2 h-2 bg-blue-400 rounded-full mt-2 mr-3 flex-shrink-0"></span>
              <span>
                Apresente o protagonista, conflito principal e stakes da
                história
              </span>
            </li>
            <li className="flex items-start">
