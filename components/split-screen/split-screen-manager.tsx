  ...
  const addReference = (type: ReferenceType, id: string) => {
    ...
    switch (type) {
      ...
      case "location":
        ...
        break;
      case "scene":
        const scene = project.manuscript
          .flatMap(
            (ch: { scenes: { id: string; title: string }[] }) => ch.scenes
          )
          .find((s: { id: string; title: string }) => s.id === id);
        if (scene) {
    ...
    if (
      item &&
      !referenceItems.find((ref) => ref.id === id && ref.type === type)
    ) {
    ...
    if (activeReference === key) {
      setActiveReference(
        referenceItems.length > 1
          ? `${referenceItems[0].type}-${referenceItems[0].id}`
          : null
      );
    }
  };
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <h1 className="text-lg font-semibold text-gray-800">
                Modo Tela Dividida - {currentScene?.title}
              </h1>
            </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              >
                ...
                <SelectContent>
                  ...
                  {project.manuscript.flatMap(
                    (ch: { scenes: { id: string; title: string }[] }) =>
                      ch.scenes
                        .filter(
          ...
          {referenceItems.length === 0 ? (
            <div className="h-full flex items-center justify-center text-gray-500">
              <div className="text-center">
                ...
                <p className="text-sm">
                  Use o menu acima para adicionar personagens, locais ou cenas
                </p>
              </div>
            </div>
          ) : (
