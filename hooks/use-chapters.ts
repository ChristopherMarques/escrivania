...
export function useChapter(chapterId: string, userId: string) {
  return useQuery({
    ...
    queryFn: async (): Promise<Chapter> => {
      const response = await fetch(
        `/api/chapters/${chapterId}?userId=${userId}`
      );
      if (!response.ok) {
...
export function useCreateChapter() {
  ...
  return useMutation({
    mutationFn: async (
      data: ChapterInsert & { userId: string }
    ): Promise<Chapter> => {
    ...
    onSuccess: (newChapter) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["chapters", newChapter.project_id],
      });
...
export function useUpdateChapter() {
  ...
  return useMutation({
    ...
    onSuccess: (updatedChapter) => {
      ...
      // Invalidar a lista de capítulos do projeto
      queryClient.invalidateQueries({
        queryKey: ["chapters", updatedChapter.project_id],
      });
    },
  });
}
  ...
  return useMutation({
    ...
    onSuccess: (_, { id }) => {
      ...
      if (cachedChapter) {
        // Invalidar a lista de capítulos do projeto
        queryClient.invalidateQueries({
          queryKey: ["chapters", cachedChapter.project_id],
        });
...
export function useReorderChapters() {
  ...
  return useMutation({
    ...
    onSuccess: (_, { chapters }) => {
      ...
      const firstChapter = queryClient.getQueryData<Chapter>([
        "chapter",
        chapters[0]?.id,
      ]);
      if (firstChapter) {
        queryClient.invalidateQueries({
          queryKey: ["chapters", firstChapter.project_id],
        });
      }
    },
  });
