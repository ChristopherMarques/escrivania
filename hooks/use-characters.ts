...
export function useCharacters(projectId: string, userId: string) {
  return useQuery({
    ...
    queryFn: async (): Promise<Character[]> => {
      const response = await fetch(`/api/characters?projectId=${projectId}&userId=${userId}`)
      if (!response.ok) {
...
export function useCharacter(characterId: string, userId: string) {
  return useQuery({
    ...
    queryFn: async (): Promise<Character> => {
      const response = await fetch(`/api/characters/${characterId}?userId=${userId}`)
      if (!response.ok) {
...
export function useCreateCharacter() {
  ...
  return useMutation({
    mutationFn: async (data: CharacterInsert & { userId: string }): Promise<Character> => {
    ...
    onSuccess: (newCharacter) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['characters', newCharacter.project_id] })
...
export function useUpdateCharacter() {
  ...
  return useMutation({
    mutationFn: async ({ id, data, userId }: { id: string; data: CharacterUpdate; userId: string }): Promise<Character> => {
    ...
    onSuccess: (updatedCharacter) => {
      ...
      // Invalidar a lista de personagens do projeto
      queryClient.invalidateQueries({ queryKey: ['characters', updatedCharacter.project_id] })
    },
  })
}
...
export function useDeleteCharacter() {
  ...
  return useMutation({
    mutationFn: async ({ id, userId }: { id: string; userId: string }): Promise<{ projectId: string }> => {
      ...
      const characterResponse = await fetch(`/api/characters/${id}?userId=${userId}`)
    ...
    onSuccess: (_, { id }) => {
      ...
      if (cachedCharacter) {
        // Invalidar a lista de personagens do projeto
        queryClient.invalidateQueries({ queryKey: ['characters', cachedCharacter.project_id] })
      }
    },
  })
}
...
export function useSearchCharacters(projectId: string, userId: string, searchTerm: string = '') {
  return useQuery({
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        ...
        withDescription: characters.filter(c => c.description && c.description.trim().length > 0).length,
        withoutDescription: characters.filter(c => !c.description || c.description.trim().length === 0).length,
        averageDescriptionLength: characters.reduce((acc, c) => {
