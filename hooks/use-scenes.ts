  ...
  return useQuery({
    ...
    queryFn: async (): Promise<Scene[]> => {
      const response = await fetch(
        `/api/scenes?chapterId=${chapterId}&userId=${userId}`
      );
      if (!response.ok) {
...
export function useCreateScene() {
  ...
  return useMutation({
    mutationFn: async (
      data: SceneInsert & { userId: string }
    ): Promise<Scene> => {
    ...
    onSuccess: (newScene) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["scenes", newScene.chapter_id],
      });
  ...
  return useMutation({
    ...
    onSuccess: (updatedScene) => {
      ...
      queryClient.setQueryData(
        ["scenes", updatedScene.chapter_id],
        (oldScenes: Scene[] | undefined) => {
          if (!oldScenes) return [updatedScene];
          return oldScenes.map((scene) =>
      ...
      queryClient.setQueryData(
        ["scenes-by-project", updatedScene.project_id],
        (oldData: any[] | undefined) => {
          if (!oldData) return undefined;
          return oldData.map((scene) =>
            scene.id === updatedScene.id
              ? { ...updatedScene, chapter_title: scene.chapter_title }
              : scene
          );
        }
      );
  ...
  return useMutation({
    ...
    onSuccess: (_, { id }) => {
      ...
      if (cachedScene) {
        // Invalidar a lista de cenas do capítulo
        queryClient.invalidateQueries({
          queryKey: ["scenes", cachedScene.chapter_id],
        });
      }
      // Invalidar a lista de cenas por projeto
      queryClient.invalidateQueries({ queryKey: ["scenes-by-project"] });
...
export function useReorderScenes() {
  ...
  return useMutation({
    ...
    onSuccess: (_, { scenes }) => {
      ...
      const firstScene = queryClient.getQueryData<Scene>([
        "scene",
        scenes[0]?.id,
      ]);
      if (firstScene) {
        queryClient.invalidateQueries({
          queryKey: ["scenes", firstScene.chapter_id],
        });
      }
    },
  });
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<(Scene & { chapter_title: string })[]> => {
      ...
      const chaptersResponse = await fetch(
        `/api/chapters?projectId=${projectId}&userId=${userId}`
      );
      ...
      const scenesPromises = chapters.map(async (chapter: any) => {
        const scenesResponse = await fetch(
          `/api/scenes?chapterId=${chapter.id}&userId=${userId}`
        );
