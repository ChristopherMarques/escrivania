...
export function useSynopses(projectId: string, userId: string) {
  return useQuery({
    ...
    queryFn: async (): Promise<Synopsis[]> => {
      const response = await fetch(`/api/synopses?projectId=${projectId}&userId=${userId}`)
      if (!response.ok) {
...
export function useSynopsis(synopsisId: string, userId: string) {
  return useQuery({
    ...
    queryFn: async (): Promise<Synopsis> => {
      const response = await fetch(`/api/synopses/${synopsisId}?userId=${userId}`)
      if (!response.ok) {
...
export function useCreateSynopsis() {
  ...
  return useMutation({
    mutationFn: async (data: SynopsisInsert & { userId: string }): Promise<Synopsis> => {
    ...
    onSuccess: (newSynopsis) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['synopses', newSynopsis.project_id] })
...
export function useUpdateSynopsis() {
  ...
  return useMutation({
    mutationFn: async ({ id, data, userId }: { id: string; data: SynopsisUpdate; userId: string }): Promise<Synopsis> => {
    ...
    onSuccess: (updatedSynopsis) => {
      ...
      queryClient.setQueryData(['synopsis', updatedSynopsis.id], updatedSynopsis)
      // Invalidar a lista de sinopses do projeto
      queryClient.invalidateQueries({ queryKey: ['synopses', updatedSynopsis.project_id] })
    },
  })
}
...
export function useDeleteSynopsis() {
  ...
  return useMutation({
    mutationFn: async ({ id, userId }: { id: string; userId: string }): Promise<{ projectId: string }> => {
      ...
      const synopsisResponse = await fetch(`/api/synopses/${id}?userId=${userId}`)
    ...
    onSuccess: (_, { id }) => {
      ...
      const cachedSynopsis = queryClient.getQueryData<Synopsis>(['synopsis', id])
      ...
      if (cachedSynopsis) {
        // Invalidar a lista de sinopses do projeto
        queryClient.invalidateQueries({ queryKey: ['synopses', cachedSynopsis.project_id] })
      }
    },
  })
}
...
export function useSearchSynopses(projectId: string, userId: string, searchTerm: string = '') {
  return useQuery({
    ...
    queryFn: async (): Promise<Synopsis[]> => {
      ...
      if (searchTerm.trim()) {
        return data.synopses.filter(synopsis => 
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        ...
        averageLength: synopses.reduce((acc, s) => {
          return acc + (s.content ? s.content.length : 0)
        }, 0) / synopses.length || 0,
        longestSynopsis: synopses.reduce((longest, current) => {
          return (current.content?.length || 0) > (longest.content?.length || 0) ? current : longest
        }, synopses[0] || null),
        shortestSynopsis: synopses.reduce((shortest, current) => {
          return (current.content?.length || 0) < (shortest.content?.length || 0) ? current : shortest
        }, synopses[0] || null)
      }
    },
    enabled: !!projectId && !!userId,
