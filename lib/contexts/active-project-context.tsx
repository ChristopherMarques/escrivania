...
type ActiveProjectAction =
  ...
  | {
      ...
      payload: { sceneId: string; content: object };
    }
  | {
      type: "UPDATE_SCENE_SYNOPSIS";
      payload: { sceneId: string; synopsis: string };
    }
  | {
      type: "UPDATE_SCENE_STATUS";
      payload: { sceneId: string; status: IScene["status"] };
    }
  ...
  | {
      ...
      payload: { originalId: string; newScene: IScene };
    }
  ...
  switch (action.type) {
    ...
    case "DELETE_CHARACTER":
      return {
        ...
        projectData: {
          ...state.projectData!,
          characters: state.projectData!.characters.filter(
    ...
    case "UPDATE_LOCATION":
      return {
        ...
        projectData: {
          ...state.projectData!,
          locations: state.projectData!.locations.map((loc) =>
    ...
    case "DELETE_LOCATION":
      return {
        ...
        projectData: {
          ...state.projectData!,
          locations: state.projectData!.locations.filter(
    ...
    case "DELETE_CHAPTER":
      return {
        ...
        projectData: {
          ...state.projectData!,
          manuscript: state.projectData!.manuscript.filter(
        ...
        projectData: {
          ...
          manuscript: state.projectData!.manuscript.map((chapter) =>
            chapter.id === action.payload.chapterId
              ? {
                  ...chapter,
                  scenes: [...chapter.scenes, action.payload.scene],
                }
              : chapter
          ),
        },
        ...
        projectData: {
          ...
          manuscript: state.projectData!.manuscript.map((chapter) =>
            chapter.scenes.some((s) => s.id === action.payload.originalId)
              ? {
                  ...chapter,
                  scenes: [...chapter.scenes, action.payload.newScene],
                }
              : chapter
          ),
        },
...
export function ActiveProjectProvider({
      ...
      const saveData = async () => {
        try {
          localStorage.setItem(
            `escrivania-project-${state.projectData!.id}`,
  ...
  const loadProject = (projectId: string) => {
    ...
    try {
      ...
      if (stored) {
        ...
        const firstScene = projectData.manuscript?.flatMap(
          (c: IChapter) => c.scenes
        )[0];
        if (firstScene) {
          dispatch({
            type: "SET_SELECTED_ITEM",
            payload: { type: "scene", id: firstScene.id },
          });
        }
      } else {
        ...
        dispatch({
          type: "SET_SELECTED_ITEM",
          payload: { type: "scene", id: "scene-1-1" },
        });
      }
    } catch (error) {
  ...
  const updateSceneContent = (sceneId: string, content: object) => {
  ...
  const duplicateScene = (sceneId: string) => {
    const scene = state.projectData?.manuscript
      .flatMap((c) => c.scenes)
      .find((s) => s.id === sceneId);
    if (scene) {
      const now = new Date().toISOString();
      ...
      dispatch({
        type: "DUPLICATE_SCENE",
        payload: { originalId: sceneId, newScene },
      });
    }
  };
  ...
  const getCurrentScene = (): IScene | undefined => {
    if (
      !state.projectData ||
      !state.selectedItem ||
      state.selectedItem.type !== "scene"
    ) {
      return undefined;
    }
    return state.projectData.manuscript
      .flatMap((c) => c.scenes)
      .find((s) => s.id === state.selectedItem!.id);
  };
  ...
  const getCurrentCharacter = (): ICharacter | undefined => {
    if (
      !state.projectData ||
      !state.selectedItem ||
      state.selectedItem.type !== "character"
    ) {
      return undefined;
    }
    return state.projectData.characters.find(
      (c) => c.id === state.selectedItem!.id
    );
  };
  ...
  const getCurrentLocation = (): ILocation | undefined => {
    if (
      !state.projectData ||
      !state.selectedItem ||
      state.selectedItem.type !== "location"
    ) {
      return undefined;
    }
    return state.projectData.locations.find(
      (l) => l.id === state.selectedItem!.id
    );
  };
