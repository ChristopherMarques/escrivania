      ...
      new Plugin({
        ...
        props: {
          handleKeyDown: (view, event) => {
            ...
            if (
              event.key === "Backspace" &&
              this.options.enableSmartBackspace
            ) {
              ...
              if ($from.parentOffset === 0) {
                ...
                if (
                  ...
                  if (prevNode) {
                    ...
                    if (
                      prevNode.type === schema.nodes.image &&
                      this.options.enableImageDeletion
                    ) {
                      const tr = state.tr.delete(
                        $from.pos - prevNode.nodeSize,
                        $from.pos
                      );
                      dispatch(tr);
                      return true;
                    ...
                    if (
                      prevNode.type === schema.nodes.paragraph &&
                      this.options.enableSmartMerge
                    ) {
            ...
            if (event.key === "Delete" && this.options.enableSmartDelete) {
              ...
              if ($from.parentOffset === node.content.size) {
                ...
                if (nextNode) {
                  ...
                  if (
                    nextNode.type === schema.nodes.image &&
                    this.options.enableImageDeletion
                  ) {
                    const tr = state.tr.delete(
                      $from.pos,
                  ...
                  if (
                    nextNode.type === schema.nodes.paragraph &&
                    nextNode.content.size === 0 &&
                    this.options.enableSmartMerge
                  ) {
                    const tr = state.tr.delete(
                      $from.pos,
                      $from.pos + nextNode.nodeSize + 1
                    );
                    dispatch(tr);
                    return true;
                  }
