...
export const PageFormat = Extension.create<PageFormatOptions>({
    ...
    return [
      {
        ...
        attributes: {
          pageSize: {
            default: 'default',
            parseHTML: element => {
              const pageSize = element.getAttribute('data-page-size') || 'default'
              this.storage.pageSize = pageSize as PageSize
              return pageSize
            },
            renderHTML: attributes => {
              const pageSize = attributes.pageSize || this.storage.pageSize || 'default'
              this.storage.pageSize = pageSize as PageSize
              return {
  ...
  onUpdate() {
    ...
    const { editor } = this
    const docPageSize = editor.getAttributes('doc').pageSize || this.storage.pageSize || 'default'
    const editorElement = editor.view.dom
